[⬅ Chemical-Equilibrium06](Chemical-Equilibrium06.md) | [🏠 Index](index.md) | [Chemical-Equilibrium08 ➡](Chemical-Equilibrium08.md)

---

# Chemical Equilibrium - Slides 31-35

## Slide 31
![Chemical Equilibrium-031](file:///home/azaz/AntigravityData/Chemistry-Semester-Final/ResourcesGivenByTeachers/Chemical-Equilibrium/Chemical%20Equilibrium-031.png)

### Text Content
**Effect of Variables on Equilibrium**

**Table 8.3: Effect of concentration, pressure, temperature, catalyst and Inert gas on equilibrium**

| Condition | Stress | Direction in which equilibrium shifts |
| :--- | :--- | :--- |
| **Concentration** | Addition of reactants (increase in reactant concentration) | Forward reaction |
| | Removal of products (decrease in product concentration) | Forward reaction |
| | Addition of products (increase in product concentration) | Reverse reaction |
| | Removal of reactants (decrease in reactant concentration) | Reverse reaction |
| **Pressure** | Increase of pressure (Decrease in volume) | Reaction that favours fewer moles of the gaseous molecules |
| | Decrease of pressure (Increase in volume) | Reaction that favours more moles of the gaseous molecules |
| **Temperature** (Alters equilibrium constants) | Increase (High T) | Towards endothermic reaction |
| | Decrease (Low T) | Towards exothermic reaction |
| **Catalyst** (Speeds up attainment of equilibrium) | Addition of catalyst | No effect |
| **Inert gas** | Addition of inert gas at constant volume | No effect |

---

## Slide 32
![Chemical Equilibrium-032](file:///home/azaz/AntigravityData/Chemistry-Semester-Final/ResourcesGivenByTeachers/Chemical-Equilibrium/Chemical%20Equilibrium-032.png)

### Text Content
**Effect of Catalysts**

### Diagram Analysis: Energy Profile
The graph plots **Energy** vs **Reaction Progress** for a reaction path:
- **Black Curve (No Catalyst)**: High activation energy ($E_a$).
- **Red Curve (With Catalyst)**: Lower activation energy ($E_a$). The catalyst provides an alternative pathway with multiple lower-energy intermediate steps.
- **Key Observation**: The energy of reactants ($X, Y$), the energy of products ($Z$), and the overall free energy change ($\Delta G$) remain identical. A catalyst speeds up both the forward and reverse reactions equally, thus it does not change the position of the equilibrium.

**Catalysis Definition**: A catalyst speeds up a reaction by lowering the activation energy required for the reaction to proceed.

---

## Slide 33
![Chemical Equilibrium-033](file:///home/azaz/AntigravityData/Chemistry-Semester-Final/ResourcesGivenByTeachers/Chemical-Equilibrium/Chemical%20Equilibrium-033.png)

### Text Content
**Factors Affecting Chemical Equilibrium - Animation**

This slide serves as a placeholder for an interactive animation exploring LeChâtelier’s Principle. The background visuals depict molecular structures involved in phase transitions and bonding, emphasizing the molecular-level adjustments that occur when a system is stressed.

---

## Slide 34
![Chemical Equilibrium-034](file:///home/azaz/AntigravityData/Chemistry-Semester-Final/ResourcesGivenByTeachers/Chemical-Equilibrium/Chemical%20Equilibrium-034.png)

### Text Content
**The Haber Process**

**A brief summary of the Haber Process**
The Haber Process combines nitrogen from the air with hydrogen derived mainly from natural gas (methane) into ammonia. The reaction is reversible and the production of ammonia is exothermic.

$$N_2(g) + 3 H_2(g) \rightleftharpoons 2 NH_3(g) \quad \Delta H = -92 \text{ kJ mol}^{-1}$$

### Diagram Analysis: Flow Scheme
The flow scheme outlines the industrial cycle:
1. **Inputs**: Nitrogen (from air) and Hydrogen (from natural gas) in a **1:3 volume ratio**.
2. **Reactor**: Operating at **400 - 450 °C** and **200 atm** pressure with an **iron catalyst**.
3. **Condensation**: The mixture is cooled; ammonia liquefies and is collected as **liquid ammonia**.
4. **Recycling**: Unreacted nitrogen and hydrogen are recycled back into the reactor to maximize efficiency.

---

## Slide 35
![Chemical Equilibrium-035](file:///home/azaz/AntigravityData/Chemistry-Semester-Final/ResourcesGivenByTeachers/Chemical-Equilibrium/Chemical%20Equilibrium-035.png)

### Text Content
**The Haber Process Apparatus**

### Diagram Analysis: Industrial Setup
The apparatus visualizes the engineering application of Le Châtelier's Principle:
- **Continuous Removal**: Ammonia is cooled in a refrigerated unit and removed as a liquid. According to Le Châtelier's Principle, removing a product ($NH_3$) shifts the equilibrium to the **right**, promoting further production.
- **Heat Management**: Heat exchangers and preheaters are used to maintain the optimal temperature range ($460\text{--}550 ^\circ C$) for the catalyst while conserving energy.
- **Compression**: High pressure is maintained to favor the side with fewer gas moles (2 moles of $NH_3$ vs 4 moles of reactants).


---

[⬅ Chemical-Equilibrium06](Chemical-Equilibrium06.md) | [🏠 Index](index.md) | [Chemical-Equilibrium08 ➡](Chemical-Equilibrium08.md)
